...
class Pager
{
    ...
    {
        if ($data === null) {
            throw new InvalidParamException(Yii::t("app", "The value of data is required."));
        }
        $this->data = $data;
        $this->run();
    }
        ...
        if ($this->numberOfPages > 1) { // Only data with multi pages
            ...
            foreach ($this->pages as $currentPageIndex => $currentPage) {
                if ($currentPageIndex === $firstPageIndex) { // First page
                    ...
                    // Add links container
                    $paginatedData .= Html::beginTag('div', ['class' => 'form-action']);
                    // Add Next link
                    ...
                    $paginatedData .= Html::beginTag('fieldset');
                    // Add the content
                    $paginatedData .= $currentPage;
                } else { // Mid pages
